...
public final class SqlScript {
	...
	public static final String GET_DEACTIVATED_GROUPS_BY_CLIENT = "SELECT B.ID BID, B.NAME, B.STATUS, C.ID CID FROM BRAND B, CLIENT C WHERE B.CLIENT_ID = C.ID AND C.ID = :ID AND B.STATUS = FALSE";
	
	public static final String GET_ACTIVATED_GROUPS_BY_CLIENT = "SELECT B.ID BID, B.NAME, B.STATUS, C.ID CID FROM BRAND B, CLIENT C WHERE B.CLIENT_ID = C.ID AND C.ID = :ID AND B.STATUS = TRUE";
}
